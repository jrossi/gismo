...
func NewLintingHandlerWithConfig(config LintingConfig) *LintingHandler {
	...
	handler := &LintingHandler{
		BaseActionHandler: gismo.NewBaseActionHandler("linting", 100),
		linters:           []linters.Linter{},
		executor:          linters.NewParallelExecutor(maxWorkers),
		config:            gismo.NewAppConfig(),
	}
...
func (h *LintingHandler) SetConfig(config *gismo.AppConfig) {
	h.config = config
	...
	if config != nil {
		for _, linter := range h.linters {
			...
			if linterConfig, ok := config.GetLinterConfig(linter.Name()); ok {
				...
				if configurable, ok := linter.(gismo.ConfigurableLinter); ok {
...
func (h *LintingHandler) ShouldHandle(ctx context.Context, event gismo.HookMessage) bool {
	switch msg := event.(type) {
	case *gismo.PreToolUseMessage:
		return msg.ToolName == "Write" || msg.ToolName == "Edit" || msg.ToolName == "MultiEdit"
	case *gismo.PostToolUseMessage:
		return msg.ToolName == "Write" || msg.ToolName == "Edit" || msg.ToolName == "MultiEdit"
	default:
		return false
	}
...
func (h *LintingHandler) HandlePreToolUse(ctx context.Context, msg *gismo.PreToolUseMessage) (*gismo.HookResponse, error) {
...
func (h *LintingHandler) HandlePostToolUse(ctx context.Context, msg *gismo.PostToolUseMessage) (*gismo.HookResponse, error) {
...
func (h *LintingHandler) formatLintOutput(filePath string, issues []linters.Issue, isBlocking bool) string {
