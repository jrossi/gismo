...
func NewNotificationHandlerWithConfig(config *NotificationConfig) *NotificationHandler {
	...
	return &NotificationHandler{
		BaseActionHandler: engine.NewBaseActionHandler("notification", 50), // Low priority
		config:            config,
	}
}

// ShouldHandle determines if this handler should process notification events
func (h *NotificationHandler) ShouldHandle(ctx context.Context, event engine.HookMessage) bool {
	_, ok := event.(*engine.NotificationMessage)
	return ok
}

// HandleNotification processes system notifications
func (h *NotificationHandler) HandleNotification(ctx context.Context, msg *engine.NotificationMessage) (*engine.HookResponse, error) {
...
func (h *NotificationHandler) ruleMatches(rule NotificationRule, msg *engine.NotificationMessage) bool {
...
func (h *NotificationHandler) logNotification(msg *engine.NotificationMessage, level, customMessage string) {
...
func (h *NotificationHandler) alertNotification(msg *engine.NotificationMessage, level, customMessage string) {
	timestamp := time.Now().Format(h.config.TimestampFormat)
	message := customMessage
	if message == "" {
		message = msg.Message
